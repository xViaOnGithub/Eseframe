...
$execute store result storage eseframe:cache tick.registry.recipe.crafting.RecipeCheckResult byte 1 run function eseframe:util/list_loop/run {pass_index:false,function:"eseframe:registry/recipe/crafting/steps/01_check_ingredient",list_path:'storage eseframe:registry recipe[{id:"$(id)",namespace:"$(namespace)"}].ingredient_item_predicates'}

# Return fail if ingredient check failed
execute if predicate {condition:"minecraft:value_check",value:{type:"minecraft:storage",storage:"eseframe:cache",path:"tick.registry.recipe.crafting.RecipeCheckResult"},range:0} run return fail
