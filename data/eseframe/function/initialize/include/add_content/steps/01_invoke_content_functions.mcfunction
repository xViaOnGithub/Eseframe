...
$execute if predicate {condition:"minecraft:value_check",value:{type:"minecraft:storage",storage:"eseframe:cache",path:"initialize.include.add_content.Step01.ContentFunctionSuccess"},range:1} run say [Eseframe API] Error! Function `$(namespace):eseframe_api-v1/content` is missing or failed!
